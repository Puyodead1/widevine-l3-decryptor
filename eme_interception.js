...
EmeInterception.onOperation = function (operationType, args) {
  if (operationType == "GenerateRequestCall") {
    ...
    console.log("last recieved request: ", lastReceivedLicenseRequest);
    console.log("last recieved response", lastReceivedLicenseResponse);

    // OK, let's try to decrypt it, assuming the response correlates to the request
    WidevineCrypto.decryptContentKey(
      lastReceivedLicenseRequest,
      lastReceivedLicenseResponse
    );
  }
};
...
EmeInterception.prototype.addListenersToNavigator_ = function () {
  ...
  navigator.requestMediaKeySystemAccess = function () {
    ...
    var result = originalRequestMediaKeySystemAccessFn.apply(
      null,
      modifiedArguments
    );
 ...
 * @private
 */
EmeInterception.prototype.addListenersToMediaKeySystemAccess_ = function (
...
EmeInterception.prototype.addListenersToMediaKeys_ = function (mediaKeys) {
  ...
  var originalCreateSessionFn = EmeInterception.extendEmeMethod(
    mediaKeys,
...
EmeInterception.prototype.addListenersToMediaKeySession_ = function (session) {
  ...
  session.generateRequest = EmeInterception.extendEmeMethod(
    session,
    session.generateRequest,
    "GenerateRequestCall"
  );
  session.load = EmeInterception.extendEmeMethod(
    session,
    session.load,
    "LoadCall"
  );
  session.update = EmeInterception.extendEmeMethod(
    session,
    session.update,
    "UpdateCall"
  );
  session.close = EmeInterception.extendEmeMethod(
    session,
    session.close,
    "CloseCall"
  );
  session.remove = EmeInterception.extendEmeMethod(
    session,
    session.remove,
    "RemoveCall"
  );
...
EmeInterception.prototype.addEmeEventListeners_ = function (element) {
  ...
  if (this.prefixedEmeEnabled) {
    element.addEventListener(
      "webkitneedkey",
      EmeInterception.interceptEvent.bind(null, "NeedKeyEvent")
    );
    element.addEventListener(
      "webkitkeymessage",
      EmeInterception.interceptEvent.bind(null, "KeyMessageEvent")
    );
    element.addEventListener(
      "webkitkeyadded",
      EmeInterception.interceptEvent.bind(null, "KeyAddedEvent")
    );
    element.addEventListener(
      "webkitkeyerror",
      EmeInterception.interceptEvent.bind(null, "KeyErrorEvent")
    );
  }

  element.addEventListener(
    "encrypted",
    EmeInterception.interceptEvent.bind(null, "EncryptedEvent")
  );
  element.addEventListener(
    "play",
    EmeInterception.interceptEvent.bind(null, "PlayEvent")
  );
...
EmeInterception.prototype.addEmeMethodListeners_ = function (element) {
  ...
  element.play = EmeInterception.extendEmeMethod(
    element,
    element.play,
    "PlayCall"
  );
  ...
  if (this.prefixedEmeEnabled) {
    element.canPlayType = EmeInterception.extendEmeMethod(
      element,
      element.canPlayType,
      "CanPlayTypeCall"
    );
    ...
    element.webkitAddKey = EmeInterception.extendEmeMethod(
      element,
      element.webkitAddKey,
      "AddKeyCall"
    );
    element.webkitCancelKeyRequest = EmeInterception.extendEmeMethod(
      element,
  ...
  if (this.unprefixedEmeEnabled) {
    element.setMediaKeys = EmeInterception.extendEmeMethod(
      element,
      element.setMediaKeys,
      "SetMediaKeysCall"
    );
  }
  ...
  for (var i = 0; i < options.length; i++) {
    ...
    if (videoCapabilities != null) {
      for (var j = 0; j < videoCapabilities.length; j++)
        if (videoCapabilities[j]["robustness"] == undefined)
          videoCapabilities[j]["robustness"] = "SW_SECURE_CRYPTO";
    }

    if (audioCapabilties != null) {
      for (var j = 0; j < audioCapabilties.length; j++)
        if (audioCapabilties[j]["robustness"] == undefined)
          audioCapabilties[j]["robustness"] = "SW_SECURE_CRYPTO";
    }
